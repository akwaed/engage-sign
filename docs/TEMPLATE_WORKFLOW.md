...
3. For Word rendering, install LibreOffice on the application host. The application tries `soffice` on `PATH`; set `LIBREOFFICE_BIN` to its absolute path if needed. If GoDaddy does not permit this binary, use an approved isolated conversion service before enabling DOCX previews. Do not send participant data to a third-party converter without approval.
