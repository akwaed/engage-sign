...
To preserve the live rows, Preview now has a narrowly scoped, administrator-only `/admin/migration-003` action. Deploy its commit, open that page while signed in as administrator, and apply it once. It checks the existing users and template records, adds only `final_archives` and the four retry columns/index, and verifies that existing row counts did not change. The action is idempotent if interrupted. Verify the new table and columns afterward, then remove the temporary migration route in a subsequent deployment. Preview and Published share the same database, so this migration affects both environments.

The mail code includes an administrator SMTP connection test at `/admin/mail`, but it has not been run on GoDaddy. A connection test does not send a message. Before production mail, configure a scheduled invocation of the mail run action or operate it manually, test delivery with an authorized test recipient, and verify domain authentication. Public DNS currently publishes an SPF record whose include chain reaches Microsoft 365, but no DMARC record or Microsoft 365 DKIM selector CNAMEs were found. Obtain the exact DKIM targets from the Microsoft 365 tenant and add/verify DKIM and DMARC before production sending.
