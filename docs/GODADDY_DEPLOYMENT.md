...
Migration `003_final_archive_and_mail.sql` and its application code are prepared, but **migration 003 has not been applied to GoDaddy**. First obtain a fresh granular **Everything** export of all 16 tables, verify it includes template rows and all table definitions, preserve the untouched copy, then append migration 003 to a corrected copy and import that full file. Verify the original administrator and template rows plus `final_archives` and the new outbox columns before updating Preview. As of September 23, the GoDaddy export dialog reported completion, but the in-app browser did not deliver a new local `.sql` file; the previous backup must not be reused. Do not deploy the new application code against the old schema.
