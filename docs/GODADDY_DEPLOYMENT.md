...
GoDaddy Preview is connected to `codex/signing-workflow`; the latest SMTP diagnostic running in Preview is commit `b604460`. Its build and site status were Healthy/OK after the preceding update. The shared hosted database has 17 tables after applying migrations 001, 002, and 003; the original administrator row remains active. The app has not been published live.

The untouched, granular 14-table export is in the local Downloads folder with SHA-256 `4D8891287BC078C3BAD799CDC82E431AF29DEC2BA7A30402E9780864B59598E2`. GoDaddy's export switch has a misleading accessibility label: visually, blue and positioned toward **Everything** includes rows; the opposite position creates a schema-only file. The final 16-table import is under ignored `work/deployment/engage-sign-full-migrated-recovered.sql` with SHA-256 `58237AD1E388989B2D2878031092D1D8466DC112B5737AA5FABC276D9B1AAAB2`. **That file predates the template uploads and is not a current restore point.** Neither SQL file belongs in Git. GoDaddy's export discarded four audit-detail JSON objects and one legal-export scope object. The objects were reconstructed from the exact application event payloads; each audit event's SHA-256 hash verified after recovering its original millisecond timestamp. The legal-export scope was restored from the application code.

Preview secrets now include `ENCRYPTION_KEY_V1` and all Microsoft 365 SMTP values, including the password entered directly by the account owner. Preview was restarted to load the secret. The encryption key has a machine-protected local backup under ignored `work/deployment/`. All nine exact baselines were uploaded and hash-verified in GoDaddy private storage. Six Word revisions have saved role-assigned field maps; the three scanned PDF baselines have draft coordinate maps. A medication-completion PDF test-fill generated successfully. The September 23 full export shows CA Contractor Agreement v2 as active; the other eight families have no active version. A Word test-fill on deployed `470f183` failed with `LibreOffice is unavailable on this host`, confirming that `soffice` is absent from `PATH`. A verified converter is required before using Word templates to send documents. The Preview health endpoint, SMTP delivery, and signing flow still need end-to-end checks. Preview and Published share the same database, so do not send real participant data or publish live until the production release gate passes.

Before applying migration 003, a granular **Everything** export of all 16 then-current tables was saved as `C:\Users\Elnoel\Downloads\engage-sign (2).sql` (SHA-256 `E67D07468575AB2FEA68143840679C4AC423E32992E259963356920237034880`). It contains the template rows, but GoDaddy's exporter flattens JSON arrays and objects, including signer plans and audit details, into invalid strings such as `[object Object]`. **Do not import this file or use it as a restore point.** The previous backup also predates the template uploads and must not be reused.

To preserve the live rows, migration 003 was applied through a narrowly scoped, administrator-only action in Preview. It added `final_archives` and the four retry columns/index without reimporting the damaged export. The application verified the unchanged row counts afterward: 1 user, 9 templates, 15 template versions, 343 mapped fields, and 28 audit events. The temporary migration action was removed from source after success. Preview and Published share the same database, so this migration affects both environments.

The mail code includes an administrator SMTP connection test at `/admin/mail`; it does not send a message. All required `SMTP_*` secret names were visible as deployed in GoDaddy Preview, and the Microsoft 365 mailbox has SMTP Authentication enabled. Preview commit `b604460` is running and its connection test reports `ESOCKET; stage CONN; TCP failed:EACCES`. A separate raw TCP probe returned `EACCES` while opening the socket, before SMTP greeting, STARTTLS, or authentication. The same Microsoft endpoint accepts a TCP connection from the local workstation, so this strongly points to the GoDaddy Node.js runtime or its network policy. The result does not validate the configured SMTP secret values or mailbox password. Ask GoDaddy Node.js Hosting support whether this app can make outbound TCP connections to `smtp.office365.com:587` and request an approved egress path; provide app ID `slgknrt6e0`, the exact error, and the UTC test time. Do not disable TLS or certificate validation or switch to GoDaddy's unauthenticated cPanel relay without a separate security review. If this hosting product cannot allow SMTP egress, use a supported HTTPS mail API with separately provisioned credentials. No message has been queued or sent. Before production mail, resolve the connection failure, configure a scheduled invocation of the mail run action or operate it manually, test delivery with an authorized test recipient, and verify domain authentication. Public DNS currently publishes an SPF record whose include chain reaches Microsoft 365, but no DMARC record or Microsoft 365 DKIM selector CNAMEs were found. Obtain the exact DKIM targets from the Microsoft 365 tenant and add/verify DKIM and DMARC before production sending.

### Email quarantine diagnosis (2026-09-23)

GoDaddy Advanced Email Security quarantined a manually forwarded Outlook message with a signing subject as spam and marked it `DKIM Signed: No`. This message was not sent by the Preview app, whose SMTP connection still fails with `EACCES`. The classification report did not identify a specific spam rule, so the missing signature is an actionable configuration gap, not a proven sole cause. GoDaddy DNS currently contains an SPF record, but no `_dmarc` TXT record and no DKIM records among its 26 records. [GoDaddy's Microsoft 365 with Advanced Email Security instructions](https://www.godaddy.com/help/enable-and-add-dkim-to-my-domain-for-microsoft-365-41748) require **both** Microsoft 365 DKIM (two tenant-specific CNAMEs) and Proofpoint Advanced Email Security DKIM (a generated TXT key). Obtain each exact key from its administrator portal, publish it in this domain's GoDaddy DNS, and enable/verify signing in the originating portal. Do not synthesize DKIM targets from examples. After both signing paths work, add a monitored DMARC policy using the organization's chosen reporting mailbox. Retest with a fresh, direct message and inspect its authentication results. Forwarding a private signing link, especially from a preview hostname, is not a valid production delivery test. If a direct, authenticated message is still quarantined, use [GoDaddy's Log Search](https://www.godaddy.com/help/access-and-release-quarantined-emails-using-advanced-email-security-32047) to identify the exact rule and report a false positive; do not disable spam protection or broadly allowlist the domain.
