...
Migration `003_final_archive_and_mail.sql` and its application code are prepared, but **migration 003 has not been applied to GoDaddy**. A fresh granular **Everything** export of all 16 tables was saved as `C:\Users\Elnoel\Downloads\engage-sign (2).sql` (SHA-256 `E67D07468575AB2FEA68143840679C4AC423E32992E259963356920237034880`). It contains the template rows, but GoDaddy's exporter flattens JSON arrays and objects, including signer plans and audit details, into invalid strings such as `[object Object]`. **Do not import this file or append migration 003 to it.** The previous backup also predates the template uploads and must not be reused.
...
The mail code includes an administrator SMTP connection test at `/admin/mail`; it does not send a message. All required `SMTP_*` secret names were visible as deployed in GoDaddy Preview, and the Microsoft 365 mailbox has SMTP Authentication enabled. The connection test has not succeeded: a bounded probe reached the application and its runtime log reported `ESOCKET` before authentication. GoDaddy replaced the application's HTTP 502 JSON response with its own HTML error page, so the diagnostic now uses HTTP 424. No message has been queued or sent. Before production mail, resolve the outbound socket/TLS failure, configure a scheduled invocation of the mail run action or operate it manually, test delivery with an authorized test recipient, and verify domain authentication. Public DNS currently publishes an SPF record whose include chain reaches Microsoft 365, but no DMARC record or Microsoft 365 DKIM selector CNAMEs were found. Obtain the exact DKIM targets from the Microsoft 365 tenant and add/verify DKIM and DMARC before production sending.
